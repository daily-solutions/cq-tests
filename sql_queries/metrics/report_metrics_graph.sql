...
sum (cast(max(fcq2.avg_totalfreezesduration) as double precision) / NULLIF(cast(avg(fcq2.avg_freeze_count) as double precision), 0)) over (partition by sh3.clientid, sh3.testid, sh3.reportnum) as freezecount_sec_avg,
...
AND sh3.reportnum = fcq2.reportnum
