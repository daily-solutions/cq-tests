...
WHERE (testid = {{parseInt(testIdSelect.value)}} OR testid = ANY({{ table1.selectedRow.data.map(row => row.testid) }}))
AND (type = 'remote-inbound-rtp' or type = 'candidate-pair' or type = 'remote-outbound-rtp')
