...
export default class ChimeCustomUplinkPolicy implements SimulcastUplinkPolicy {
  ...
  private lastUplinkBandwidthKbps: number =
    DefaultSimulcastUplinkPolicy.defaultUplinkBandwidthKbps;
  ...
  private observerQueue: Set<SimulcastUplinkObserver> =
    new Set<SimulcastUplinkObserver>();

  constructor(
    private selfAttendeeId: string,
    private logger: Logger,
    private customSimulcast,
    private receiveSettings
  ) {
    ...
    this.parametersInEffect = new DefaultVideoCaptureAndEncodeParameter(
      0,
      0,
      0,
      0,
    ...
    this.currentQualityMap = this.fillEncodingParamWithBitrates([
      customSimulcast[0].maxBitrate,
      0,
      customSimulcast[2].maxBitrate,
    ]);
  ...
  updateConnectionMetric({ uplinkKbps = 0 }: ConnectionMetrics): void {
    ...
    if (
      ...
      DefaultSimulcastUplinkPolicy.startupDurationMs
    ) {
      this.lastUplinkBandwidthKbps =
        DefaultSimulcastUplinkPolicy.defaultUplinkBandwidthKbps;
    } else {
  ...
  private calculateEncodingParameters(
    ...
    if (
      ...
      } else if (this.numSenders < this.receiveSettings.mid) {
        this.newActiveStreams = ActiveStreams.kHiAndLow;
        newBitrates[0].maxBitrateKbps = this.customSimulcast[0].maxBitrate;
        newBitrates[1].maxBitrateKbps = 0;
        newBitrates[2].maxBitrateKbps = this.customSimulcast[2].maxBitrate;
      } else if (this.numSenders < this.receiveSettings.low) {
        // 320x192 + 640x384 + (1280x768)
        this.newActiveStreams = ActiveStreams.kMidAndLow;
        newBitrates[0].maxBitrateKbps = this.customSimulcast[0].maxBitrate;
        newBitrates[1].maxBitrateKbps = this.customSimulcast[1].maxBitrate;
        newBitrates[2].maxBitrateKbps = 0;
      } else {
        // 320x192 + 640x384 + (1280x768)
  ...
  updateIndex(videoIndex: VideoStreamIndex): void {
    ...
    const numSenders =
      videoIndex.numberOfVideoPublishingParticipantsExcludingSelf(
        this.selfAttendeeId
      ) + 1;
    ...
    const newShouldDisableSimulcast =
      numParticipants >= 0 && numParticipants <= 2;
    const shouldDisableSimulcastChanged =
    ...
    for (let i = 0; i < nameArr.length; i++) {
      ...
      newMap.set(ridName, {
        ...
        scaleResolutionDownBy:
          this.customSimulcast[i].scaleResolutionDownBy ?? 1,
  ...
  private publishEncodingSimulcastLayer(): void {
    const simulcastLayers = this.getEncodingSimulcastLayer(
      this.activeStreamsToPublish
    );
